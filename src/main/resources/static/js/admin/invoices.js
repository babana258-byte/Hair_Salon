// ── 假資料 ──
const invoices = [
  { id: 1, customer: '王小明', designer: '林設計師', date: '2025-05-04', items: '縮毛矯正', product: '摩洛哥堅果修護油', subtotal: 3500, discount: 0, total: 3580, paymentMethod: '刷卡', paidAt: '2025-05-04 14:30' },
  { id: 2, customer: '李小華', designer: '王設計師', date: '2025-05-04', items: '線條染、護髮', product: '絲滑修復洗髮露', subtotal: 4200, discount: 200, total: 4080, paymentMethod: '刷卡', paidAt: '2025-05-04 16:00' },
  { id: 3, customer: '陳小美', designer: '林設計師', date: '2025-05-03', items: '剪髮', product: '', subtotal: 500, discount: 0, total: 500, paymentMethod: '現金', paidAt: '2025-05-03 17:20' },
  { id: 4, customer: '張大偉', designer: '王設計師', date: '2025-05-02', items: '燙髮', product: '', subtotal: 2800, discount: 0, total: 2800, paymentMethod: '轉帳', paidAt: null },
  { id: 5, customer: '林小芳', designer: '陳助理', date: '2025-05-01', items: '護髮療程', product: '水潤絲絨髮膜', subtotal: 1200, discount: 100, total: 1100, paymentMethod: '現金', paidAt: '2025-05-01 13:30' },
  { id: 6, customer: '黃小玲', designer: '林設計師', date: '2025-04-30', items: '頭皮養護、剪髮', product: '頭皮養護精華', subtotal: 1800, discount: 0, total: 1800, paymentMethod: '刷卡', paidAt: '2025-04-30 12:00' },
];
...
function renderTable() {
  ...
  if (filtered.length === 0) {
    tbody.innerHTML = `<tr><td colspan="10" style="text-align:center;color:var(--neutral);padding:2rem;font-style:italic;">查無消費記錄</td></tr>`;
    return;
  }
  ...
  tbody.innerHTML = filtered.map(inv => `
    <tr>
      ...
      <td>${inv.customer}</td>
      <td style="font-size:0.82rem;">${inv.designer || '—'}</td>
      <td>${inv.date}</td>
      <td style="font-size:0.8rem;color:var(--neutral);">${inv.items}</td>
      <td style="font-size:0.8rem;color:var(--neutral);">${inv.product || '—'}</td>
      <td>
...
function applyFilter() {
  ...
  filtered = invoices.filter(inv => {
    const matchKeyword = !keyword || inv.customer.includes(keyword) || inv.items.includes(keyword) || (inv.designer || '').includes(keyword) || (inv.product || '').includes(keyword);
...
function openDetail(id) {
  ...
  document.getElementById('detailDesigner').textContent = inv.designer || '—';
  ...
  document.getElementById('detailProduct').textContent = inv.product || '—';
...
function openAddInvoice() {
  ...
  // 重置下拉
  ['iMethodDropdown', 'iPaidDropdown', 'iDesignerDropdown'].forEach(function(id) {
  ...
  document.getElementById('iDesignerLabel').textContent = '選擇設計師';
...
document.getElementById('addInvoiceForm').addEventListener('submit', function(e) {
  ...
  const newInv = {
    ...
    designer: document.getElementById('iDesigner').value,
